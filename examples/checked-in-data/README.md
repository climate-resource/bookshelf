...
`path:` is relative to the recipe and the file stays beside it, so the example is self-contained and needs no network.

The build derives one frame from the input and writes it with `used=[raw]`, which is what records the lineage edge.
