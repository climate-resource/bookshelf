...
A resource states its own catalogue metadata and inherits none of the book's, so a field nobody wrote stays unset.
The two ends of the produce path spell the fields identically: a declared input states them under `resources:` in the recipe, and a produced output passes them to `book.write`.
