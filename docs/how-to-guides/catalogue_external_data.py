...
#
# [Publishing a book](publish_a_book) registered one derived resource whose bytes the platform stores.
...
from bookshelf import Bookshelf, RegisterItem
...
pointer.tracking_id, pointer.registration_status

# %% [markdown]
# Passing `hash=` records the expected SHA256,
# so a later fetch can be verified against it.
# Without one the platform stores the pointer and cannot vouch for what is behind it.
#
...
#
# Inside an activity the same call records that this run produced the pointer,
# and `used=` records what it consumed.

# %%
with bs.activity(
    code_ref="github.com/climate-resource/bookshelf@docs",
    config={"step": "mirror-upstream"},
) as activity:
...
mirrored.tracking_id, mirrored.registration_status

# %% [markdown]
# ## Registering a batch
#
# `register_many()` takes `RegisterItem` values and registers them in one request.
# It is the right call whenever a run produces more than one output,
# because it is a single round trip and one lineage record.
...
[(str(item.tracking_id)[:8], item.registration_status) for item in outputs]

# %% [markdown]
# ## Batch limits and partial failure
#
# The batching rules are worth knowing before a large run hits them.
#
...
# Byte identical items owned by one organisation collapse to the first canonical resource,
# even when a later item supplies a different logical key.
# The first resource's logical key stays canonical.
#
# The handle reports this through `registration_status`,
# so a caller can tell a fresh registration from an aliased one.

# %%
with bs.activity(code_ref="github.com/climate-resource/bookshelf@docs") as activity:
    first = activity.register(series(9.0), type="timeseries", logical_key="demo-sdk-howto/dedupe/first")
    second = activity.register(series(9.0), type="timeseries", logical_key="demo-sdk-howto/dedupe/second")

print(f"first  {first.tracking_id} {first.registration_status}")
print(f"second {second.tracking_id} {second.registration_status}")
print(f"same resource: {first.tracking_id == second.tracking_id}")
