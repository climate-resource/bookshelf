...
def test_the_live_facade_refuses_to_resolve_a_resource() -> None:
    """Outside a recording there is no recipe, so there is nothing to resolve against."""
    with Bookshelf() as bs, pytest.raises(BookshelfError, match="no active recording"):
        bs.use("raw")
...
def test_a_version_doi_lands_on_the_recorded_pointer(tmp_path: Path, server: _Server) -> None:
    ...
    with _recording(_write_recipe(tmp_path, _URI_VERSION), bundle_path):
        bs, _ = setup()
        bs.use("raw")
        pointer = _written_pointers(bs, bundle_path)[0]
...
def test_the_handle_is_usable_as_lineage(tmp_path: Path, server: _Server) -> None:
    ...
    with _recording(_write_recipe(tmp_path, _URI_VERSION), tmp_path / "bundle"):
        ...
        recorded = {r.tracking_id: r for r in bs.bundle.manifest.resources}
